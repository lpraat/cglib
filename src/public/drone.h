...
private:
    ...
    const float32 positionSpeed = 25.0f;
...
public:
    ...
    Mat4<T> getModel() {
        return cglib::translate(position).dot(orientation.toRotMatrix());
    }
    ...
    Vec3<T> getLightDirection() const {
        const cglib::Quat<float32> d = orientation * cglib::Quat<float32> {0, 0, 0.1f, 1.0f}.normalize() * orientation.conjugate();
        return {d.x, d.y, d.z};
    }
