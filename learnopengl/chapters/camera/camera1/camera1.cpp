...
int main() {
    ...
    ShaderProgram shaderProgram(
        "/Users/lpraat/develop/computer_graphics/learnopengl/chapters/camera/camera1/shaders/vertex.vs",
        "/Users/lpraat/develop/computer_graphics/learnopengl/chapters/camera/camera1/shaders/fragment.fs"
    );
    ...
    glp::Vec3 cameraPos {0.0f, 0.0f, 3.0f};
    glp::FPSCamera<float32> camera(cameraPos);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // View matrix
        shaderProgram.use();
        glp::Mat4 newView = camera.getView();
...
void processInput(GLFWwindow* window, glp::FPSCamera<float32>& camera, float32 deltaTime) {
    ...
    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) {
        camera.updateCamera(glp::FPSCameraMovement::FORWARD, deltaTime);
    }

    if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) {
        camera.updateCamera(glp::FPSCameraMovement::BACKWARD, deltaTime);
    }

    if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
        camera.updateCamera(glp::FPSCameraMovement::LEFT, deltaTime);
    }

    if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) {
        camera.updateCamera(glp::FPSCameraMovement::RIGHT, deltaTime);
    }
}
